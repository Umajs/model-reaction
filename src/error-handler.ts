...
export class ErrorHandler {
  private errorListeners: Record<ErrorType, Array<(error: AppError) => void>> = {
  ...
  offError(type: ErrorType, listener: (error: AppError) => void): void {
    if (this.errorListeners[type]) {
      this.errorListeners[type] = this.errorListeners[type].filter(l => l !== listener);
    }
  }

  // 触发错误
  triggerError(error: AppError): void {
    console.error(`[${error.type}] ${error.field ? `字段 ${error.field}: ` : ''}${error.message}`);

    // 触发特定类型的错误监听器
    if (this.errorListeners[error.type]) {
      this.errorListeners[error.type].forEach(listener => listener(error));
    }

    // 触发通用错误监听器
    if (this.errorListeners[ErrorType.UNKNOWN]) {
      this.errorListeners[ErrorType.UNKNOWN].forEach(listener => listener(error));
    }
  }
